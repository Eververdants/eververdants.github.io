(function () {
  ...
  function initFluid(canvas, params) {
    if (!canvas) return;
    var gl;
    try {
    ...
    var decayProg = link(FLUID_VS, DECAY_FS);
    var fluidProg = link(FLUID_VS, FLUID_FS);
    if (!decayProg || !fluidProg) return;
    ...
    canvas.width = cw;
    canvas.height = ch;
    var bw = Math.round(cw / 4);
    var bh = Math.round(ch / 4);
    var flowInit = new Uint8Array(bw * bh * 4);
    ...
    var targetA = makeTarget(bw, bh, flowInit);
    var targetB = makeTarget(bw, bh, flowInit);

    var touch = window.matchMedia("(hover: none), (pointer: coarse)").matches;
    var isWindows = navigator.userAgentData
      ? navigator.userAgentData.platform === "Windows"
      : navigator.userAgent.indexOf("Windows") !== -1;
    var useMouse = !touch && !isWindows;
    ...
    function frame(now) {
      ...
      var w2 = Math.round(canvas.clientWidth * r);
      var h2 = Math.round(canvas.clientHeight * r);
      if (w2 !== cw || h2 !== ch) {
        cw = w2;
        ch = h2;
        canvas.width = cw;
      ...
      var src = ping ? targetA : targetB;
      var dst = ping ? targetB : targetA;
      ping = !ping;
  ...
  initFluid(document.getElementById("bg-fluid"), FLUID_PARAMS);

  /* ---------- dsh background: 2D dot grid ---------- */
  ...
  function initDots(canvas) {
    ...
    var LINE_COLOR = "rgba(255, 255, 255,";
    var DOT_COLOR = "rgba(255, 255, 255,";
    var LINE_OPACITY = 0.08;
    var DOT_OPACITY = 0.16;
    ...
    function build() {
      ...
      var offX = (w - (cols - 1) * 90) / 2;
      var offY = (h - (rows - 1) * 90) / 2;
      dots = [];
      for (var n = 0; n < rows; n++) {
        for (var r = 0; r < cols; r++) {
          var x = offX + 90 * r;
          var y = offY + 90 * n;
          dots.push({ restX: x, restY: y, x: x, y: y, vx: 0, vy: 0 });
        }
      }
    }
    ...
    function draw(now) {
      ...
      ctx.strokeStyle = LINE_COLOR + " " + LINE_OPACITY + ")";
      ctx.lineWidth = 0.5;

      for (var n = 0; n < rows; n++) {
        for (var c = 0; c < cols - 1; c++) {
          var a = dots[n * cols + c];
          var b = dots[n * cols + c + 1];
          var ox = b.x - a.x;
          var oy = b.y - a.y;
          ...
          var uy2 = oy / l;
          ctx.beginPath();
          ctx.moveTo(a.x + 10 * ux2, a.y + 10 * uy2);
          ctx.lineTo(b.x - 10 * ux2, b.y - 10 * uy2);
          ctx.stroke();
        }
      }
      for (var c2 = 0; c2 < cols; c2++) {
        for (var n2 = 0; n2 < rows - 1; n2++) {
          var a2 = dots[n2 * cols + c2];
          var b2 = dots[(n2 + 1) * cols + c2];
          var ox2 = b2.x - a2.x;
          var oy2 = b2.y - a2.y;
          var l2 = Math.sqrt(ox2 * ox2 + oy2 * oy2);
          if (l2 < 20) continue;
          var ux3 = ox2 / l2;
          var uy3 = oy2 / l2;
          ctx.beginPath();
          ctx.moveTo(a2.x + 10 * ux3, a2.y + 10 * uy3);
          ctx.lineTo(b2.x - 10 * ux3, b2.y - 10 * uy3);
          ctx.stroke();
        }
      }

      ctx.fillStyle = DOT_COLOR + " " + DOT_OPACITY + ")";
      for (var i2 = 0; i2 < dots.length; i2++) {
        ...
        if (!isNaN(mx) && !isNaN(my)) {
          ...
          var f2 = Math.max(0, 1 - dist2 / 140);
          r2 = 1.8 + 2 * f2;
          a3 = DOT_OPACITY + 0.4 * f2;
        }
        ctx.globalAlpha = a3;
        var s = 2 * r2;
        ctx.fillRect(d2.x - r2, d2.y - r2, s, s);
      }
      ctx.globalAlpha = 1;
