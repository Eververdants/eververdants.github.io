/* WebGL2 fluid background — migrated from Astro src/scripts/landing.js,
   then enriched: multi-octave domain-warped noise (two dyes) plus a
...
export function initFluid(
  ...
  /* Per-screen atmosphere ramp: each section owns a tint, anchored to its
     DOM element. Measured as a sorted list of document-Y keyframes, then
     re-measured on fonts.ready (big-type shifts layout), resize, and load —
     the handscroll sets its height late, which moves the blog/outro stops.
     Two timeouts catch GSAP's post-init height set without importing gsap. */
  let stops: { y: number; atmosphere: Atmosphere }[] = [];

  function measureStops() {
    stops = ATMOSPHERE_STOPS.map((s) => {
      if (!s.sel) return { y: 0, atmosphere: s.atmosphere };
      const el = document.querySelector<HTMLElement>(s.sel);
      if (!el) return { y: Number.POSITIVE_INFINITY, atmosphere: s.atmosphere };
      const r = el.getBoundingClientRect();
      let y = r.top + window.scrollY;
      if (s.anchor === "center") y += r.height / 2;
      else if (s.anchor === "bottom") y += r.height;
      return { y, atmosphere: s.atmosphere };
    })
  ...
  function frame(now: number) {
    ...
    /* Sample the per-screen atmosphere at the current scroll position and
       feed every palette/light uniform straight from it. Each atmosphere
       carries its own light/bloom/gain, so the warm sections read dimmer and
       the finale fades toward black — no single cyan→orange mix, no global
       dim factor. */
    const atm = sampleAtmosphere(stops, window.scrollY);
    ...
    gl.uniform1f(flu.vignette, atm.vignette);
    gl.uniform1f(flu.bloomThreshold, params.bloomThreshold);
    gl.uniform1f(flu.bloomRange, params.bloomRange);
    gl.uniform1f(flu.bloomStrength, atm.bloomStrength);
    gl.uniform1f(flu.glowIntensity, atm.glowIntensity);

    const g1 = hexToRgb(atm.glowColors[0]);
    const g2 = hexToRgb(atm.glowColors[1]);
    const g3 = hexToRgb(atm.glowColors[2]);
    ...
    for (let ci = 0; ci < 5; ci++) {
      const rgb = hexToRgb(atm.colors[ci] || atm.colors[atm.colors.length - 1]);
